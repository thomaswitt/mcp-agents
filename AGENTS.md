...
- The Codex tools expose closed wrapper-owned schemas. `codex` requires `prompt`, an **absolute** `cwd`, `sandbox`, and `model_reasoning_effort` (`xhigh|max`), with optional `goal`; `codex-reply` requires `prompt` and nonblank `threadId`, with optional `goal`. `additionalProperties` is false. Native `model`, raw `config`, `approval-policy`, direct developer/base/compact instructions, reply sandbox/cwd/effort, missing fields, and malformed values are rejected locally with redacted JSON-RPC `-32602` responses and never reach Codex. Approval policy is server-owned (`never` by default, startup-overridable)
- `transformCodexToolCall` only receives already-validated calls. It translates an initial session's wrapper effort to native `config.model_reasoning_effort` and strips/injects wrapper-only `goal`: initial goals become native `developer-instructions`, while reply goals become prompt reminders. Codex's native `/goal` is TUI-only. Accepted frames with nothing to transform (notably a goal-free `codex-reply`) MUST remain byte-for-byte unchanged. The stdin pump therefore buffers raw bytes and splits only on newline byte `0x0a`
