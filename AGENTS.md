...
- Native subagents are OFF by default and session-scoped. The isolated config writes BOTH `[features] multi_agent = false` AND `[agents] enabled = false` — on Codex >= 0.145.0 the stabilized feature flag alone does NOT remove the collab tools (verified empirically; only `agents.enabled = false` does), while older versions still honor the feature flag. A `codex`/`codex-start` call with `allow_subagents: true` re-enables them for that session only via per-call config overrides (`agents.enabled` + `features.multi_agent`); replies inherit it like `sandbox` and reject the arg. The `[mcp_servers]` strip is untouched either way, so enabled subagents remain Codex-only in-process workers with no bridge re-entry or cross-LLM reach; they inherit the session's `sandbox_mode` + `approval_policy` (shared-workspace concurrency is the residual caveat). Multi-agent V2 reports lifecycle work as `sub_agent_activity` (narrated in progress alongside the older `collab_agent_*` names)
